...
export const Route = createFileRoute("/services/automation")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: `${p.oneLine} AI agents, CRM workflows and reporting that connect marketing, sales and operations.` },
      { property: "og:title", content: `${p.label} — Flogrit` },
      { property: "og:description", content: `${p.oneLine} The third pillar of the Flogrit growth system.` },
    ],
  }),
    ...
    <>
      <section className="border-b border-border bg-background">
        <div className="mx-auto max-w-7xl px-5 py-24 lg:px-8 lg:py-32">
          <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-muted-foreground">03 · {p.label}</p>
          <h1 className="mt-4 max-w-4xl font-display text-5xl font-semibold leading-[1.02] tracking-[-0.02em] md:text-6xl lg:text-7xl">
            {p.oneLine.replace(/\.$/, "")} — <span className="text-primary">quietly, in the background.</span>
          </h1>
