          ...
          {features.map((feat, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`relative bg-[#13131A] border rounded-2xl p-7 flex flex-col gap-4 transform ${rotClasses[idx % 6]} ${feat.highlight ? 'border-[#3B82F6]/40' : 'border-white/10'}`}
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-wrap mt-1">
                  <h3 className="font-display font-black text-xl text-white">{feat.title}</h3>
                  {feat.highlight && (
                    <span className="bg-[#3B82F6] text-white text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded">
                      Pro only
                    </span>
                  )}
                </div>
